...
function filterMapReduce() {
  ...
  const samplesOver5 = samples.filter(function (x) {
    return x > 5;
  });
  const squaresOfSamplesOver5 = samplesOver5.map(function (x) {
    return x * x;
  });
...
function generateCompanyEmail({ fullName: name, company='foobar' }) {
